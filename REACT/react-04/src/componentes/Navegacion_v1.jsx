...
export const Navegacion_v1 = (propiedades) => {
    ...
    const subMenuHandler = (elnavLinksData ,  i , elementos , subMenuId , mostrarSubMenu) => {
        setShowSubMenu((prev) => {            
            ...
            // hago un bucle for que rellene un array 'arr' con los valores 'undefined' y luego en esa posición
            // lo pongo a true
    ...
    const showItemHandler = (elSubMenu , menuId , subMenuId , mostrarSubMenu) => {
        setShowSubMenu((prev) => {            
            ...
            for (let index = 0; index <elSubMenu.length; index++){
                    ...
                    <>
                        <div key={el.id} className="px-3" onClick={(event) => subMenuHandler(navLinksData ,  i , el , el.id , true)}>
                            ...
                            {showSubMenu[el.id] &&
                                el.división.map((ele) => {
                                    return (
                                        <div id={ele.id} key={ele.id} className="btn p-0 rounded border-0 m-0" onClick={(event) => showItemHandler( el.división , el.id , ele.id , true)}>
